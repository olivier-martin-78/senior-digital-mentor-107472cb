-- Modifier la fonction pour utiliser les logs d'authentification pour obtenir la vraie dernière connexion
...
BEGIN
  ...
  SELECT 
    ...
    p.email,
    p.created_at,
    -- Utiliser les logs d'authentification pour obtenir la vraie dernière connexion
    COALESCE(
      (SELECT MAX((log_data->>'time')::timestamp with time zone)
       FROM (
         SELECT jsonb_path_query(
           CASE 
             WHEN event_message::jsonb ? 'auth_event' THEN event_message::jsonb->'auth_event'
             ELSE event_message::jsonb
           END, 
           '$.actor_username'
         )::text as actor_email,
         event_message::jsonb->'time' as log_time,
         event_message::jsonb as log_data
         FROM auth_logs 
         WHERE event_message ILIKE '%login%' 
           AND event_message ILIKE '%' || p.email || '%'
           AND timestamp > NOW() - INTERVAL '30 days'
       ) logs 
       WHERE logs.actor_email = '"' || p.email || '"'),
      au.last_sign_in_at
    ) as last_sign_in_at,
    p.display_name,
