-- Fix session counting by using only user_actions for session calculation
-- Add debug logs specifically for Pinsan to understand the counting issue
...
CREATE OR REPLACE FUNCTION public.get_dashboard_stats(
  ...
  content_type_param text DEFAULT NULL::text, 
  action_type_param text DEFAULT NULL::text
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  -- 7. NOUVELLE LOGIQUE: Sessions = jours d'activité (user_actions seulement)
  ...
  WITH user_activity_days AS (
    ...
    HAVING COUNT(DISTINCT DATE(ua.timestamp)) > 0
