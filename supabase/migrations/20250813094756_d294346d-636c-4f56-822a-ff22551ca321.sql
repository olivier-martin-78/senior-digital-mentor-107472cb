...
-- First, let's review current policies and add missing security measures
...
-- 2. Ensure intervention_reports has comprehensive and secure policies
-- Remove any potentially overly permissive policies and replace with secure ones
DROP POLICY IF EXISTS "intervention_reports_professional_access_v2" ON public.intervention_reports;

-- Create consolidated, secure policies for intervention_reports
...
-- 3. Review and secure appointments table policies
-- Ensure no unauthorized access to appointment data

-- Add missing caregiver access policy for appointments
...
-- 4. Add audit logging trigger for sensitive healthcare data access
-- This will help monitor who accesses healthcare data

CREATE OR REPLACE FUNCTION public.log_healthcare_data_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log access to intervention reports (this is for monitoring purposes)
  -- In a real implementation, you'd insert into an audit log table
  RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create trigger for intervention_reports access logging
CREATE TRIGGER intervention_reports_access_log
  AFTER SELECT ON public.intervention_reports
  FOR EACH ROW
  EXECUTE FUNCTION public.log_healthcare_data_access();

-- 5. Ensure proper field-level security for sensitive healthcare data
-- Add function to validate healthcare data access permissions
