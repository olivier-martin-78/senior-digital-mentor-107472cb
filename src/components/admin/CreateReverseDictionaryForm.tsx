...
interface CreateReverseDictionaryFormProps {
  onSuccess: () => void;
  onCancel: () => void;
}

const CreateReverseDictionaryForm: React.FC<CreateReverseDictionaryFormProps> = ({
  onSuccess,
  onCancel
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      toast({
        title: 'Erreur',
        description: 'Vous devez être connecté pour créer un dictionnaire inversé',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      // Track creation
      UserActionsService.trackCreate('activity', 'reverse-dictionary-created', formData.title, {
        action: 'reverse_dictionary_created',
      ...
      toast({
        title: 'Succès',
        description: 'Dictionnaire inversé créé avec succès',
      });

      onSuccess();
    } catch (error) {
      console.error('Erreur lors de la création du dictionnaire inversé:', error);
      toast({
        title: 'Erreur',
        description: 'Impossible de créer le dictionnaire inversé',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={loading}>
              {loading ? 'Création...' : 'Créer le dictionnaire inversé'}
            </Button>
...
export default CreateReverseDictionaryForm;