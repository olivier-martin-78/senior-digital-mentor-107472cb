...
const LifeStoryPermissions: React.FC<Props> = ({ storyOwnerId, storyOwnerName }) => {
  const { user } = useAuth();
  const [permissions, setPermissions] = useState<LifeStoryPermission[]>([]);
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">
          Permissions pour l'histoire de vie de {storyOwnerName}
        </h3>
      ...
      {permissions.length > 0 ? (
        ...
        <p className="text-gray-500 text-center py-4">
          Aucune permission accordée pour cette histoire de vie
        </p>
      )}
    </div>
  );
...
export default LifeStoryPermissions;
