...
// Create helper to check connection status - simplified for auth context
export const checkConnection = async (): Promise<boolean> => {
  try {
    // Pour le contexte d'authentification, on fait un test plus simple
    // qui ne dépend pas de l'état d'authentification
    ...
    console.log('🔍 Health check failed, trying alternative method...');
    
    // Fallback: essayer une requête basique vers Supabase
    try {
      const start = Date.now();
      const { error } = await supabase.from('profiles').select('count').limit(0);
      const end = Date.now();
      console.log(`🔍 Alternative connection check took ${end - start}ms`, { success: !error, error });
      return !error;
    } catch (e) {
      console.error('🔍 All connection checks failed:', e);
      return false;
    }
  }
};

// Helper specifically for auth confirmation that doesn't require authentication
export const checkAuthConnection = async (): Promise<boolean> => {
  try {
    // Test simple pour vérifier que Supabase répond
    ...
    console.log('🔍 Auth connection check:', response.status, response.ok);
    return response.ok;
  } catch (e) {
