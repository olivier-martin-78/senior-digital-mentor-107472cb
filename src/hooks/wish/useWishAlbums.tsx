  ...
  useEffect(() => {
    const fetchAlbums = async () => {
      ...
      try {
        ...
        console.log('🔍 useWishAlbums - Récupération avec logique de groupe CORRIGÉE');
        ...
        if (userGroupsError) {
          console.error('❌ useWishAlbums - Erreur récupération groupes:', userGroupsError);
          setAlbums([]);
          setLoading(false);
          return;
        }

        console.log('👥 useWishAlbums - Groupes de l\'utilisateur:', userGroupMemberships);
        ...
        if (userGroupMemberships && userGroupMemberships.length > 0) {
          ...
          for (const membership of userGroupMemberships) {
            ...
            if (groupCreator && !authorizedUserIds.includes(groupCreator)) {
              ...
              console.log('✅ useWishAlbums - Ajout du créateur du groupe:', groupCreator);
            }
          }
        ...
        console.log('📁 useWishAlbums - Albums récupérés:', {
          count: data?.length || 0,
          albums: data?.map(a => ({
            id: a.id,
            name: a.name,
